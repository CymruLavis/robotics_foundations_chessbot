...
# pose = None
input_linkname = 'p4'
# # input_linkname = ['p4', 'p3', 'n6', 'b2', 'q3', 'k4']
...
def get_links_gazebo(link_states_msg):
    # Call back to retrieve the object you are interested in
    # global input_linkname
    link_index = link_states_msg.name.index(input_linkname)
    link_pose = link_states_msg.pose[link_index]
    rospy.loginfo("Link pose: {}".format(link_pose))

def main():
    rospy.init_node('get_link_pose', anonymous=True)
